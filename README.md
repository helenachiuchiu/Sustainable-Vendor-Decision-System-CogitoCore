...
The system employs a multi-agent architecture coordinated by a central `MessageBus` for agent-to-agent communication (arrows in the above diagram represent the Message Bus/A2A Protocol). Six specialized agents—`DataCollectionAgent`, `ESGAgent`, `RiskAnalysisAgent`, `TOPSISRankingAgent`, `ValidationAgent`, and `MemoryAgent`—work collaboratively, each handling distinct tasks. External tools (`GoogleSearchTool`, `GoogleSheetsTool`) enable data enrichment and persistence. The `MultiAgentOrchestrator` manages execution flow, supporting parallel and sequential processing. A Streamlit-based UI provides vendor management, evaluation configuration, and results visualization, with comprehensive logging ensuring full system observability.
...
### Conclusion
This multi-agent system delivers a robust, end‑to‑end pipeline for sustainable vendor selection. The `MessageBus` and `MultiAgentOrchestrator` enable modular, parallel, and traceable collaboration between specialized agents for data collection, ESG scoring, risk analysis, quantitative ranking, ethical validation, and long‑term memory. External integrations (Google Search, Google Sheets, Gemini) enrich vendor profiles and preserve institutional knowledge, while the TOPSIS‑based ranking ensures decisions remain quantitatively rigorous and configurable. The `ValidationAgent` embeds ESG safeguards directly into the optimization loop, and the Streamlit UI exposes this intelligence through an accessible, auditable interface. Overall, the architecture balances automation, transparency, and ethical constraints to support defensible, sustainability‑aligned procurement decisions.
...
![Add new vendor](/Images/Vendor_Mgt_Add_new.png)
