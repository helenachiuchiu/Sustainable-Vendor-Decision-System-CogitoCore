...
def vendor_mgmt_page():
    ...
    tab1, tab2 = st.tabs(["📋 View Vendors", "➕ Add Vendor"])
    ...
    with tab1:
        ...
        with col1:
            del_id = st.text_input("Delete Vendor by ID")
    ...
    with tab2:
        with st.form("add_vendor"):
            col1, col2 = st.columns(2)
            with col1:
                v_id = st.text_input("ID", f"V{len(st.session_state.vendors)+1:03d}")
                v_name = st.text_input("Name")
                v_cost = st.number_input("Cost ($)", min_value=0.0, value=1000.0)
                v_quality = st.number_input("Quality (0-100)", 0, 100, 80)
            with col2:
                v_delivery = st.number_input("Delivery (days)", 1, 365, 10)
                v_risk = st.number_input("Risk (0-100)", 0, 100, 20)
...
def main():
    st.set_page_config(
        page_title="Sustainable Vendor Decision System",
        page_icon="🏭",
        layout="wide"
    )
